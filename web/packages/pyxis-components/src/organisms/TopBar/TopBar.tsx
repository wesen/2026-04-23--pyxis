...
export const TopBar = ({ breadcrumb, title, subtitle, actions }: TopBarProps) => (
  <div {...pyxisPart('top-bar')} style={{ display: 'flex', justifyContent: 'space-between', alignItems: subtitle ? 'flex-start' : 'center', gap: 16, padding: '20px 24px', background: 'var(--color-surface)', borderBottom: '1px solid var(--color-border)' }}>
    <div {...pyxisPart('top-bar', 'title-group')}>
    ...
    {actions && <div {...pyxisPart('top-bar')} style={{ display: 'flex', gap: 8 }}>{actions}</div>}
  </div>
);
