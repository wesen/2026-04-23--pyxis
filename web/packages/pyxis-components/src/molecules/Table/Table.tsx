...
export function Table<T>({ columns, rows, onRowClick }: TableProps<T>) {
  return (
    <div {...pyxisPart('table')} style={{ width: '100%', overflowX: 'auto' }}>
      <table {...pyxisPart('table', 'table')} style={{ width: '100%', borderCollapse: 'collapse', fontSize: 'var(--text-sm)' }}>
        <thead {...pyxisPart('table', 'head')}>
