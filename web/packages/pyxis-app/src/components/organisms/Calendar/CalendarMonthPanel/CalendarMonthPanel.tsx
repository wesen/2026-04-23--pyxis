  ...
  return (
    <section className="app-panel app-calendar-month-panel" data-section="calendar-month" data-pyxis-component="panel" data-pyxis-part="root">
      ...
      <div className="app-calendar-month-grid">{cells.map((day, index) => {
        ...
        return <div key={index} className="app-calendar-cell" data-empty={!day || undefined} data-today={date === today || undefined} data-selected={date === selectedDate || undefined} onClick={day ? () => onSelectDate?.(date) : undefined}>{day && <><button className="app-calendar-day-button" type="button" onClick={(clickEvent) => { clickEvent.stopPropagation(); onSelectDate?.(date); }}><b>{day}</b><span>{dayEvents.length === 0 ? 'Open night' : `${dayEvents.length} event${dayEvents.length === 1 ? '' : 's'}`}</span></button>{dayEvents.map((event) => <button key={`${event.kind}-${event.id}-${event.date}-${event.label}`} className="app-calendar-event-button" type="button" onClick={(clickEvent) => { clickEvent.stopPropagation(); onEventClick?.(event); }}><CalendarEventChip event={event}/></button>)}</>}</div>;
      })}</div>
      <CalendarLegend />
    </section>
  );
