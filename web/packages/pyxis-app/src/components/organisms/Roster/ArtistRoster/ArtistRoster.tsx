    ...
    <div {...appPart('artist-roster')}>
      {artists.length > 0 ? <>
        <div className="app-card-list app-mobile-only">
          {artists.map((artist)=><button aria-label={`Select ${artist.name}`} className="app-artist-roster-button" key={artist.id} onClick={() => onSelectArtist?.(artist)}><ArtistCard artist={artist}/></button>)}
        </div>
