...
export function DashboardUpcomingPanel({ shows, onViewAll }: DashboardUpcomingPanelProps) {
  ...
  return <Panel title="Upcoming shows" kicker="Pinned to #upcoming-shows" action={<Button variant="outline" size="sm" onClick={onViewAll}>View all ›</Button>} section="dashboard-upcoming"><div className="app-desktop-only"><ShowsTable shows={visible} variant="dashboard"/></div><div className="app-stack app-mobile-only">{shows.slice(0,3).map((show) => <TodayShowCard key={show.id} show={show}/>)}</div>{shows.length === 0 && <AppEmptyState title="No confirmed shows yet." />}</Panel>;
}
