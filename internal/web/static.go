...
func NewSPAHandler() http.Handler {
	content, err := publicContentFS()
	if err != nil {
		return missingBundleHandler(err)
	}
...
func (h *spaHandler) serveIndex(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		http.Error(w, "public site bundle is missing index.html; run `make build-web`", http.StatusServiceUnavailable)
		return
	}
...
func missingBundleHandler(err error) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if isReservedPath(r.URL.Path) {
			http.NotFound(w, r)
			return
		}
		http.Error(w, "public site bundle is unavailable; run `make build-web`: "+err.Error(), http.StatusServiceUnavailable)
	})
}
