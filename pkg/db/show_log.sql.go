// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.22.0
// source: show_log.sql

package db

import (
	"context"

	"github.com/jackc/pgx/v5/pgtype"
)

const getShowLog = `-- name: GetShowLog :one
SELECT id, show_id, draw, notes, incident, incident_notes, logged_by, created_at, updated_at, quick_highlight, total_door_cents FROM show_logs WHERE show_id = $1
`

func (q *Queries) GetShowLog(ctx context.Context, showID int32) (ShowLog, error) {
	row := q.db.QueryRow(ctx, getShowLog, showID)
	var i ShowLog
	err := row.Scan(
		&i.ID,
		&i.ShowID,
		&i.Draw,
		&i.Notes,
		&i.Incident,
		&i.IncidentNotes,
		&i.LoggedBy,
		&i.CreatedAt,
		&i.UpdatedAt,
		&i.QuickHighlight,
		&i.TotalDoorCents,
	)
	return i, err
}

const listShowLogs = `-- name: ListShowLogs :many
SELECT al.id, al.show_id, al.draw, al.notes, al.incident, al.incident_notes, al.logged_by, al.created_at, al.updated_at, al.quick_highlight, al.total_door_cents, s.artist, s.date
FROM show_logs al
JOIN shows s ON s.id = al.show_id
ORDER BY s.date DESC
LIMIT $1 OFFSET $2
`

type ListShowLogsParams struct {
	Limit  int32 `json:"limit"`
	Offset int32 `json:"offset"`
}

type ListShowLogsRow struct {
	ID             int32              `json:"id"`
	ShowID         int32              `json:"showId"`
	Draw           pgtype.Int4        `json:"draw"`
	Notes          pgtype.Text        `json:"notes"`
	Incident       pgtype.Bool        `json:"incident"`
	IncidentNotes  pgtype.Text        `json:"incidentNotes"`
	LoggedBy       pgtype.Int4        `json:"loggedBy"`
	CreatedAt      pgtype.Timestamptz `json:"createdAt"`
	UpdatedAt      pgtype.Timestamptz `json:"updatedAt"`
	QuickHighlight pgtype.Text        `json:"quickHighlight"`
	TotalDoorCents pgtype.Int4        `json:"totalDoorCents"`
	Artist         string             `json:"artist"`
	Date           pgtype.Date        `json:"date"`
}

func (q *Queries) ListShowLogs(ctx context.Context, arg ListShowLogsParams) ([]ListShowLogsRow, error) {
	rows, err := q.db.Query(ctx, listShowLogs, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListShowLogsRow{}
	for rows.Next() {
		var i ListShowLogsRow
		if err := rows.Scan(
			&i.ID,
			&i.ShowID,
			&i.Draw,
			&i.Notes,
			&i.Incident,
			&i.IncidentNotes,
			&i.LoggedBy,
			&i.CreatedAt,
			&i.UpdatedAt,
			&i.QuickHighlight,
			&i.TotalDoorCents,
			&i.Artist,
			&i.Date,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const upsertShowLog = `-- name: UpsertShowLog :one
INSERT INTO show_logs (show_id, draw, notes, incident, incident_notes, logged_by, quick_highlight, total_door_cents)
VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
ON CONFLICT (show_id) DO UPDATE SET
    draw = EXCLUDED.draw,
    notes = EXCLUDED.notes,
    incident = EXCLUDED.incident,
    incident_notes = EXCLUDED.incident_notes,
    logged_by = EXCLUDED.logged_by,
    quick_highlight = EXCLUDED.quick_highlight,
    total_door_cents = EXCLUDED.total_door_cents,
    updated_at = NOW()
RETURNING id, show_id, draw, notes, incident, incident_notes, logged_by, created_at, updated_at, quick_highlight, total_door_cents
`

type UpsertShowLogParams struct {
	ShowID         int32       `json:"showId"`
	Draw           pgtype.Int4 `json:"draw"`
	Notes          pgtype.Text `json:"notes"`
	Incident       pgtype.Bool `json:"incident"`
	IncidentNotes  pgtype.Text `json:"incidentNotes"`
	LoggedBy       pgtype.Int4 `json:"loggedBy"`
	QuickHighlight pgtype.Text `json:"quickHighlight"`
	TotalDoorCents pgtype.Int4 `json:"totalDoorCents"`
}

func (q *Queries) UpsertShowLog(ctx context.Context, arg UpsertShowLogParams) (ShowLog, error) {
	row := q.db.QueryRow(ctx, upsertShowLog,
		arg.ShowID,
		arg.Draw,
		arg.Notes,
		arg.Incident,
		arg.IncidentNotes,
		arg.LoggedBy,
		arg.QuickHighlight,
		arg.TotalDoorCents,
	)
	var i ShowLog
	err := row.Scan(
		&i.ID,
		&i.ShowID,
		&i.Draw,
		&i.Notes,
		&i.Incident,
		&i.IncidentNotes,
		&i.LoggedBy,
		&i.CreatedAt,
		&i.UpdatedAt,
		&i.QuickHighlight,
		&i.TotalDoorCents,
	)
	return i, err
}
