...
UPDATE shows AS s SET status = 'archived', updated_at = NOW() WHERE s.id = $1 RETURNING s.id, s.artist, s.date, s.doors_time, s.start_time, s.age, s.price, s.genre, s.description, s.notes, s.status, s.flyer_url, s.discord_message_id, s.discord_channel_id, s.submission_id, s.artist_id, s.created_by, s.created_at, s.updated_at, s.draw, s.capacity, s.reserve_ticket_enabled, s.google_cal_event_id, s.google_cal_synced_at
`
...
WHERE s.id = $1
RETURNING s.id, s.artist, s.date, s.doors_time, s.start_time, s.age, s.price, s.genre, s.description, s.notes, s.status, s.flyer_url, s.discord_message_id, s.discord_channel_id, s.submission_id, s.artist_id, s.created_by, s.created_at, s.updated_at, s.draw, s.capacity, s.reserve_ticket_enabled, s.google_cal_event_id, s.google_cal_synced_at
`
...
SELECT id, artist, date, doors_time, start_time, age, price, genre, description, notes, status, flyer_url, discord_message_id, discord_channel_id, submission_id, artist_id, created_by, created_at, updated_at, draw, capacity, reserve_ticket_enabled, google_cal_event_id, google_cal_synced_at FROM shows WHERE id = $1
`
...
SELECT id, artist, date, doors_time, start_time, age, price, genre, description, notes, status, flyer_url, discord_message_id, discord_channel_id, submission_id, artist_id, created_by, created_at, updated_at, draw, capacity, reserve_ticket_enabled, google_cal_event_id, google_cal_synced_at FROM shows
...
SELECT id, artist, date, doors_time, start_time, age, price, genre, description, notes, status, flyer_url, discord_message_id, discord_channel_id, submission_id, artist_id, created_by, created_at, updated_at, draw, capacity, reserve_ticket_enabled, google_cal_event_id, google_cal_synced_at FROM shows ORDER BY date DESC
`
...
const updateShowGoogleCalSync = `-- name: UpdateShowGoogleCalSync :one
UPDATE shows
...
type UpdateShowGoogleCalSyncParams struct {
	ID                int32              `json:"id"`
	GoogleCalEventID  string             `json:"googleCalEventId"`
	GoogleCalSyncedAt pgtype.Timestamptz `json:"googleCalSyncedAt"`
}

func (q *Queries) UpdateShowGoogleCalSync(ctx context.Context, arg UpdateShowGoogleCalSyncParams) (Show, error) {
	row := q.db.QueryRow(ctx, updateShowGoogleCalSync, arg.ID, arg.GoogleCalEventID, arg.GoogleCalSyncedAt)
	var i Show
	err := row.Scan(
		&i.ID,
		&i.Artist,
